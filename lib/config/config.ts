...
import { get, has, set, merge } from 'lodash'
  ...
  public getConfigFromEnv() {
    ...
    Object.keys(process.env).forEach(key => {
      if (key.startsWith(this.prefix)) {
        ...
        if (parts.length === 2) {
          // 如果长度为2，则读取的是类似lin_a.b结构的配置，envs[a][b]设置设置为process.env['lin_a.b']
          set(envs, parts[1], process.env[key])
        } else if (parts.length > 2) {
          // 长度大于2，读取的是lin_a_b结构的配置，取出a_b替换为a.b,然后将对应的环境变量的值设置到envs[a][b]中
